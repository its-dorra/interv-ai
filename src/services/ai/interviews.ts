import { fetchChatMessages } from "../hume-ai/lib/api";
import { generateText } from "ai";
import { google } from "./models/google";
import type { JobInfoTable } from "@/drizzle/schema";
...
export async function generateAiInterviewFeedback({
  ...
  const chatMessages = await fetchChatMessages(humeChatId);
  ...
  const formattedMessages = chatMessages
    .map((msg) => {
      ...
      if (!msg.messageText) return null;

      return {
        speaker: msg.type === "USER_MESSAGE" ? "interviewee" : "interviewer",
        text: msg.messageText,
        emotionFeatures: msg.role === "USER" ? msg.emotionFeatures : undefined,
      };
    })
    .filter((msg) => msg != null);
